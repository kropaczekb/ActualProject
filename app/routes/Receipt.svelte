<script>
  ...
  export let receiptObj
  console.log("receipt", receiptObj)
...
async function generatePdf() {
  ...
  let inputObj = {}
  inputObj = {
    "date": receiptObj.paymentInfo.result.payment.createdAt,
    "receipt": receiptObj.paymentInfo.result.payment.receiptNumber,
    "total": (receiptObj.paymentInfo.result.payment.amountMoney.amount / 100).toFixed(2),
    "itemCost": (receiptObj.orderInfo.totalPreCost/100).toFixed(2),
    "shippingCost": (receiptObj.orderInfo.shipping/100).toFixed(2),
  }
  receiptObj.orderInfo.itemsOrdered.forEach((product, i) => {
...
<h1>Receipt</h1>
<div>Date: {receiptObj.paymentInfo.result.payment.createdAt}</div>
<div>Amount paid: ${(receiptObj.paymentInfo.result.payment.amountMoney.amount / 100).toFixed(2)}</div>
<div>Receipt No. {receiptObj.paymentInfo.result.payment.receiptNumber}</div>

<div>Items Purchased</div>

<div id="itemContainer">
{#each receiptObj.orderInfo.itemsOrdered as item}
  <div class="item">
    {#if item.Options.secondOption !== null}
    <div>{item.Options.firstOption} {item.Options.secondOption} {item.Name}</div>
    {:else}
      <div>{item.Options.firstOption} {item.Name}</div>
    {/if}
    <div>{item.Options.quantity}</div>
    <div>${(item.Price/100).toFixed(2)}</div>
  </div>
{/each}
</div>

<div id="shipping">Shipping: ${(receiptObj.orderInfo.shipping/100).toFixed(2)}</div>
<div id="preCost">Sum total: ${(receiptObj.orderInfo.totalPreCost/100).toFixed(2)}</div>
<div id="total">Final total: ${((receiptObj.orderInfo.totalPreCost + receiptObj.orderInfo.shipping)/100).toFixed(2)}</div>

<button on:click={generatePdf}>Generate a PDF receipt</button>
